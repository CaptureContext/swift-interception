...
extension NSObject {
	...
	///          been performed.
	public func swizzle(_ pairs: (Selector, Any)..., key hasSwizzledKey: AssociationKey<Bool>) {
		...
		synchronized(subclass) {
			...
			if !subclassAssociations.value(forKey: hasSwizzledKey) {
				...
				for (selector, body) in pairs {
					...
					if method_getImplementation(method) == _swiftInterceptionMsgForward {
						...
						precondition(
							succeeds,
							"combine-interception attempts to swizzle a selector that has already a runtime injected implementation. This is unsupported in the current version."
						)
					}
				}
			}
...
public func swizzleClass(_ instance: NSObject) -> AnyClass {
...
private func subclassName(of class: AnyClass) -> String {
	return String(cString: class_getName(`class`)).appending("_combine-interceptionSwift")
}
